...
#ifndef lint
static	char	rcsid[] = "@(#)$Id: mail.c,v 1.1.1.1 1999/07/16 21:21:43 toast Exp $";
#endif
...
check_mail()
{
	...
	if (stat_buf.st_ctime > old_stat)
	{
		...
		if ((des = open(mail_path, O_RDONLY, 0)) >= 0)
		{
			...
			{
				if (!strncmp(MAIL_DELIMITER, buffer,
					sizeof(MAIL_DELIMITER) - 1))
				{
					new_cnt++;
# ifdef MAIL_TWO
					if (new_cnt>cnt && !VirginProgram &&
					    get_int_var(MAIL_VAR)==2)

					{
						/*buffer[strlen(buffer)-1]='\0';
						say("%s", buffer); */
						say("[%s] You have new email:", TimeStamp());
						while (dgets(buffer,
						    BIG_BUFFER_SIZE, des,
						    (char *) 0) > 0 &&
						    *buffer != '\0' &&
...
# endif /* AMS_MAIL */
		...
		if (new_cnt > cnt)
		{
			if (do_hook(MAIL_LIST, "%s %s", tmp, buffer) &&
			    get_int_var(MAIL_VAR)==1)
