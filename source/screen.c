...
#ifndef lint
static	char	rcsid[] = "@(#)$Id: screen.c,v 1.2 2001/09/01 01:11:21 toast Exp $";
#endif
...
/* display_blink: turns off and on the display blinking.  */
static	char
display_blink(flag)
	int	flag;
{
...
output_line(str, result, startpos)
	...
	int	startpos;
{
	static	int	high = OFF, bold = OFF, blink = OFF;
	int	rev_tog, und_tog, bld_tog, blk_tog, all_off;
	char	*ptr;
	int	len;
	int	written = 0;
	char	c;
		...
		case ALL_OFF:
			...
			if (!startpos)
			  if (len >= 0)
				...
				fprintf(stderr, "Tried to write a negative-length string!\n");
			rev_tog = und_tog = bld_tog = blk_tog = all_off = 0;
			do
			{
				switch(*ptr)
				...
				case ALL_OFF:
					all_off = 1;
					und_tog = rev_tog = bld_tog = blk_tog = 0;
					break;
				}
			} while ((ptr[1] == REV_TOG || ptr[1] == UND_TOG ||
			    ptr[1] == BOLD_TOG || ptr[1] == BLINK_TOG ||
			    ptr[1] == ALL_OFF) && ptr++);
			if (all_off)
			{
				if (!underline)
	...
	for (ptr = (u_char *) str; *ptr && (pos < BIG_BUFFER_SIZE - 8); ptr++)
	{
		 ...
		 */
				if (!cont)
				{
					...
					{
				/*	Indenting any more than 8 spaces is 
				 *	stupid if you ask me. (Toast 9/9/1997)
		 		 */
						if (indent > len+8)
							indent = len+8;
						cont = (char *)
...
#define IRCXTERM_MAX 10
	...
	say("Opening new %s...",
		...
		screen_type == ST_SCREEN ? "screen" :
					   "wound" );
	sprintf(sock.sun_path, "/tmp/irc_%08d", getpid());
	...
	default:
		 ...
		 */
		if (dgets(buffer, BIG_BUFFER_SIZE, current_screen->fdin) < 1)
		{
