...
#ifndef lint
static	char	rcsid[] = "@(#)$Id: hook.c,v 1.5 2001/11/29 19:31:11 toast Exp $";
#endif
	...
	HookFunc hook_functions[] =
{
	...
	{ "SEND_OPS",		(Hook *) 0,	2,	0,	0 },
	{ "SEND_OPS_OTHER",	(Hook *) 0,	2,	0,	0 },
...
{
	if (list->server != -1)
		say("On %s from %c%s%c do %s [%s] <%d> (Server %d)%s", name,
		...
		say("On %s from %c%s%c do %s [%s] <%d>", name,
		    (list->flexible?'\'':'"'), list->nick, (list->flexible?'\'':'"'),
		    (list->not ? "nothing" : list->stuff),
		...
		*arg6;
{
	...
	for (tmp = *list;tmp;tmp = tmp->next)
	{
		...
		if (tmp->flexible)
			tmpnick = expand_alias((char *) 0, tmp->nick, empty_string, &sa, NULL);
		else
		...
		quiet;
{
	...
	if (nick)
	{
		...
		{
			if (!quiet)
				say("%c%s%c removed from %s list", (tmp->flexible?'\'':'"'),
					nick, (tmp->flexible?'\'':'"'),
		...
		*subargs;
{
	...
	*serial;
	...
	if (get_int_var(NOVICE_VAR) && !load_depth)
	{
	    ...
	    if (!banner)
		banner = empty_string;
	    yell("%sYou may not type ON commands when you have the NOVICE",
		banner);
	    yell("%svariable set to ON. Some ON commands may cause a", banner);
	    yell("%ssecurity breach on your machine, or enable another",
		banner);
	    yell("%suser to control your IRC session. Read the help files",
		banner);
	    yell("%sin /HELP ON before using ON", banner);
	    return;
	}
...

	{
		...
		if (*nick)
		{
			...
			add_hook(which, nick, args, noisy, not, server, sernum, flexible);
			if (which < 0)
				say("On %3.3u from %c%s%c do %s [%s] <%d>",
				    -which, (flexible?'\'':'"'), nick, (flexible?'\'':'"'),
				    (not ? "nothing" : args),
				    noise_level[noisy], sernum);
			else
				say("On %s from %c%s%c do %s [%s] <%d>",
					hook_functions[which].name, (flexible?'\'':'"'),
					nick, (flexible?'\'':'"'),
					(not ? "nothing" : args),
					noise_level[noisy], sernum);
			new_free(&nick);
